...
int main()
{
	ElemType menu_FirstNum, menu_SecondNum = 1,SearchList_Check, ReverseList_Check,e = 0;  //检查是否运行成功
	Status InitList_Check, InsertList_Check,DeleteList_Check;
	LNode* ReverseEvenList_Check, *FindMidNode_Check;  //输出奇偶调换后的第一个节点 链表中点
	L = (LINK)malloc(sizeof(LinkedList));
    *L = (LinkedList)malloc(sizeof(LNode));
	while (menu_SecondNum)
	{
		system("cls");
		menu_SecondNum = FunctionMenu();  //菜单
